...
# Dumps command output for later use, currently in faraday-ide
def command_to_file(command):
    file_path_dump = '/root/command-dump_%s.txt' % (scanip)
    print bcolors.OKGREEN + "INFO: Writing command output to file:\n " + file_path_dump + bcolors.ENDC
    subprocess.check_output("echo " + "\"" + command + "\" " + ">> " + file_path_dump, shell=True)
...
def dirb(ip_address, port, url_start):
    ...
    DIRBSCAN = "dirb %s://%s:%s -o /root/oscp/exam/%s/dirb-%s.txt -r -S" % (
    url_start, ip_address, port, ip_address, ip_address)
    ...
    command_to_file(DIRBSCAN)
    return


def nikto(ip_address, port, url_start):
    print bcolors.HEADER + "INFO: Starting nikto scan for " + ip_address + bcolors.ENDC
    NIKTOSCAN = "nikto -h %s://%s -o /root/oscp/exam/%s/nikto-%s-%s.txt" % (
    url_start, ip_address, ip_address, url_start, ip_address)
    NIKTOSCAN2 = "nikto -h %s://%s -o /root/oscp/exam/%s/nikto-%s-%s.xml" % (
    url_start, ip_address, ip_address, url_start, ip_address)
...
def httpEnum(ip_address, port):
    ...
    command_to_file(CURLSCAN)
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt,http-devframework,http-enum,http-frontpage-login,http-git,http-iis-webdav-vuln,http-php-version,http-robots.txt,http-shellshock,http-vuln-cve2015-1635 -oN /root/oscp/exam/%s/%s_http.nmap -oX /root/oscp/exam/%s/%s_http.xml %s" % (
    port, ip_address, ip_address, ip_address, ip_address, ip_address)
    print bcolors.HEADER + HTTPSCAN + bcolors.ENDC
    command_to_file(HTTPSCAN)
...
def httpsEnum(ip_address, port):
    ...
    command_to_file(SSLSCAN)

    HTTPSCANS = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt,http-devframework,http-enum,http-frontpage-login,http-git,http-iis-webdav-vuln,http-php-version,http-robots.txt,http-shellshock,http-vuln-cve2015-1635 -oN /root/oscp/exam/%s/%s_https.nmap -oX /root/oscp/exam/%s/%s_https.xml %s" % (
    port, ip_address, ip_address, ip_address, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oN /root/oscp/exam/%s/mssql_%s.nmap -oX /root/oscp/exam/%s/mssql_%s.xml %s" % (
    port, ip_address, ip_address, ip_address, ip_address, ip_address)
...
def smtpEnum(ip_address, port):
    ...
    SMTPSCAN = "nmap -sV -Pn -p %s --script=smtp-commands,smtp-enum-users,smtp-vuln-cve2010-4344,smtp-vuln-cve2011-1720,smtp-vuln-cve2011-1764 %s -oN /root/oscp/exam/%s/smtp_%s.nmap -oX /root/oscp/exam/%s/smtp_%s.xml" % (
    port, ip_address, ip_address, ip_address, ip_address, ip_address)
...
def ftpEnum(ip_address, port):
    ...
    FTPSCAN = "nmap -sV -Pn -vv -p %s --script=ftp-anon,ftp-bounce,ftp-libopie,ftp-proftpd-backdoor,ftp-vsftpd-backdoor,ftp-vuln-cve2010-4221 -oN /root/oscp/exam/%s/ftp_%s.nmap -oX /root/oscp/exam/%s/ftp_%s.xml %s" % (
    port, ip_address, ip_address, ip_address, ip_address, ip_address)
    ...
    print results_ftp
    command_to_file(FTPSCAN)
    return


def udpScan(ip_address):
    print bcolors.HEADER + "INFO: Detected UDP on " + ip_address + bcolors.ENDC
    UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN /root/oscp/exam/%s/udp_%s.nmap -oX /root/oscp/exam/%s/udp_%s.xml %s" % (
    ip_address, ip_address, ip_address, ip_address, ip_address)
    ...
    command_to_file(UDPSCAN)
    UNICORNSCAN = "unicornscan -mU -v -I %s > /root/oscp/exam/%s/unicorn_udp_%s.txt" % (
...
def nmapScan(ip_address):
    ...
    TCPSCAN = "nmap -sV -O %s -oN /root/oscp/exam/%s/%s.nmap -oX /root/oscp/exam/%s/%s.xml" % (
    ip_address, ip_address, ip_address, ip_address, ip_address)
    ...
    for scanip in targets:
        ...
        if not scanip in dirs:
            ...
            subprocess.check_output(
                "cp /root/oscp/reports/windows-template.md /root/oscp/exam/" + scanip + "/mapping-windows.md",
                shell=True)
            subprocess.check_output(
                "cp /root/oscp/reports/linux-template.md /root/oscp/exam/" + scanip + "/mapping-linux.md", shell=True)
            print bcolors.OKGREEN + "INFO: Added pentesting templates: " + "/root/oscp/exam/" + scanip + bcolors.ENDC
            subprocess.check_output(
                "sed -i -e 's/INSERTIPADDRESS/" + scanip + "/g' /root/oscp/exam/" + scanip + "/mapping-windows.md",
                shell=True)
            subprocess.check_output(
                "sed -i -e 's/INSERTIPADDRESS/" + scanip + "/g' /root/oscp/exam/" + scanip + "/mapping-linux.md",
                shell=True)
